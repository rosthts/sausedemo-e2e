...
1. **PageManager** — рефакторинг `tests/fixtures/index.ts`: один клас-контейнер, що ліниво створює/кешує page objects, замість окремого fixture на кожну сторінку — наступне
2. **Config-клас** — інкапсуляція env/baseURL/timeouts
3. **Власні error-класи** для доменних ситуацій (наприклад коли очікувана помилка логіну не з'явилась)
4. Далі повертаємось до `CartPage`/`CheckoutPage`, вже на новій інфраструктурі
5. Пізніше: API-клієнт, Allure, CI matrix/sharding
